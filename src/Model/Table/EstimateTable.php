...
class EstimateTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ...
            ->add('unit_price1', 'custom', [
                'rule' => function ($value, $context) {
                    $quantity = $context['data']['quantity1'];
                    return ($value !== '' && $quantity === '') ||  ($value === '' && $quantity !== '');
                },
                'message' => '単価と数量はどちらか一方のみ入力することはできません。',
            ]);
        ...
        $validator
            ->integer('amount1')
            ->requirePresence('amount1', 'create', '単価と数量が入力されていません。')
        ...
        for ($i = 2; $i <= EstimateConst::FORM_NOT_HOSOKU; $i++) {
            ...
            $validator
                ...
                ->add('unit_price' . $i, 'custom', [
                    'rule' => function ($value, $context) use ($i) {
                        $quantity = $context['data']['quantity' . $i];
                        return ($value !== '' && $quantity === '') ||  ($value === '' && $quantity !== '');
                    },
                    'message' => '単価と数量はどちらか一方のみ入力することはできません。',
                ]);
